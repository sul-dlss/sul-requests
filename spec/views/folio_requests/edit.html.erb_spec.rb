...
RSpec.describe 'folio_requests/edit' do
  let(:request_attributes) { {} }
  let(:mock_request) { build(:request, custom_properties: request_attributes) }
  ...
  context 'when the request has a fill_by_date' do
    let(:request_attributes) { { 'expirationDate' => '2019-01-01' } }
  ...
  context 'when the request has no fill_by_date' do
    let(:request_attributes) { { 'expirationDate' => nil } }

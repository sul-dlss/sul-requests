...
RSpec.describe PatronRequest do
  ...
  describe '#display_type' do
    context 'request type scan' do
      ...
      it { expect(request.display_type).to eq 'Scan' }
    end

    context 'request type mediated' do
      let(:attr) { { request_type: 'mediated' } }

      it { expect(request.display_type).to eq 'Mediated page' }
    end
  ...
  describe '#create_aeon_requests' do
    ...
    context 'with single item digitization request' do
      ...
      it 'creates an aeon request with digitization fields' do
        expect(request.aeon_requests.length).to eq 1
        aeon_request = request.aeon_requests.first
        expect(aeon_request.special_request).to eq 'info'
        expect(aeon_request.pages).to eq '23'
        expect(aeon_request.publication).to be false
        expect(aeon_request.shipping_option).to equal 'Electronic Delivery'
      end
    end
    ...
    context 'with single item reading room request' do
      ...
      it 'creates an aeon request with reading room fields' do
        expect(request.aeon_requests.length).to eq 1
        aeon_request = request.aeon_requests.first
        expect(aeon_request.special_request).to eq 'Some info'
        expect(aeon_request.shipping_option).to be_nil
      end
    end
    ...
    context 'with multi item digitization request' do
      ...
      it 'creates multiple aeon requests with digitization fields' do
        expect(request.aeon_requests.length).to eq 2
        aeon_request = request.aeon_requests.second
        expect(aeon_request.special_request).to eq 'more info'
        expect(aeon_request.pages).to eq '32'
        expect(aeon_request.publication).to be true
      end
    end
  end
end

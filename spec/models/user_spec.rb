# frozen_string_literal: true

require 'rails_helper'

RSpec.describe User do
  subject(:user) { described_class.new }

  describe 'validations' do
    it 'onlies allow unique sunetids' do
      described_class.create!(sunetid: 'some-user')
      expect do
        described_class.create!(sunetid: 'some-user')
      end.to raise_error(ActiveRecord::RecordInvalid)
    end
  end

  describe '#sucard_number=' do
    it 'sets the library_id omitings the first 5 digits' do
      expect(user.library_id).to be_blank
      user.sucard_number = '12345987654321'
      expect(user.library_id).to eq '987654321'
    end
  end

  describe '#library_id' do
    it 'upcases the library id to match symphony' do
      user.library_id = 'somelibid'
      expect(user.library_id).to eq 'SOMELIBID'
    end
  end

  describe '#barcode' do
    it 'uses the library id' do
      user.library_id = 'somelibid'
      expect(user.barcode).to eq 'SOMELIBID'
    end

    context 'returns the library id from FOLIO' do
      let(:patron) { instance_double(Folio::Patron, library_id: '123456789') }

      it 'uses the patron barcode from the ILS' do
        allow(Folio::Patron).to receive(:find_by).and_return(patron)
        user.library_id = 'somelibid'

        expect(user.barcode).to eq '123456789'
      end
    end
  end

  describe '#email_address' do
    describe 'for SSO users' do
      before do
        user.sunetid = 'jstanford'
      end

      it 'returns the email address set by the email attribute' do
        user.email = 'jjstanford@stanford.edu'
        expect(user.email_address).to eq 'jjstanford@stanford.edu'
      end

      context 'when there is no email set from the LDAP attributes' do
        it 'sets the value to the SUNet@stanford.edu' do
          expect(user.email_address).to eq 'jstanford@stanford.edu'
        end

        it 'Notifies the exception handling service' do
          allow(Honeybadger).to receive(:notify)
          expect(user.email_address).to eq 'jstanford@stanford.edu'

          expect(Honeybadger).to have_received(:notify).with(
            'SSO User being created without an email address. Using jstanford@stanford.edu instead.'
          )
        end
      end
    end

    describe 'for non-SSO users' do
      it 'returns the user email address' do
        user.name = 'Jane Stanford'
        user.email = 'jstanford@example.com'
        expect(user.email_address).to eq 'jstanford@example.com'
      end
    end

    describe 'for library ID users' do
      it 'is blank' do
        user.library_id = '123456'
        expect(user.email_address).to be_blank
      end
    end
  end

  describe '#to_email_string' do
    describe 'for SSO users' do
      it 'is their Stanford email address' do
        user.name = 'Jane Stanford'
        user.sunetid = 'jstanford'
        user.email = 'jstanford@stanford.edu'
        expect(user.to_email_string).to eq 'Jane Stanford (jstanford@stanford.edu)'
      end
    end

    describe 'for non-SSO users' do
      it 'is their name plus their email in parenthesis' do
        user.name = 'Jane Stanford'
        user.email = 'jstanford@stanford.edu'
        expect(user.to_email_string).to eq 'Jane Stanford (jstanford@stanford.edu)'
      end
    end

    describe 'for users without a name' do
      it 'justs be their email address' do
        user.sunetid = 'jstanford'
        user.email = 'jstanford@stanford.edu'
        expect(user.to_email_string).to eq 'jstanford@stanford.edu'
      end
    end

    describe 'for library id users' do
      it 'is blank' do
        user.library_id = '123456'
        expect(user.to_email_string).to be_blank
      end
    end
  end

  describe '#sso_user?' do
    it 'returns false when the user has no sunetid attribute' do
      expect(user).not_to be_sso_user
    end

    it 'returns true when the user has a sunetid attribute' do
      user.sunetid = 'SSO user'
      expect(user).to be_sso_user
    end
  end

  describe '#student_type' do
    it 'processes the data from the database properly' do
      expect(user.student_type).to eq([])

      user.student_type = 'type1;type2'
      expect(user).to be_changed
      user.save
      expect(user.student_type).to eq %w[type1 type2]
    end
  end

  describe '#library_id_user?' do
    it 'is true when the user has supplied a library ID' do
      user.library_id = '12345'
      expect(user).to be_library_id_user
    end

    it 'is false when the user has not supplied a library ID' do
      expect(user).not_to be_library_id_user
    end
  end

  describe '#name_email_user?' do
    it 'is true when the user has supplied a name and email address' do
      user.name = 'jstanford'
      user.email = 'jstanford@stanford.edu'
      expect(user).to be_name_email_user
    end

    it 'is false when the user has not supplied a library ID' do
      expect(user).not_to be_name_email_user
    end
  end

  describe '#super_admin?' do
    it 'returns false when the user is not a super admin' do
      expect(user).not_to be_super_admin
    end

    it 'returns true when the user is in a superadmin group' do
      user.ldap_group_string = 'sul:requests-super-admin'
      expect(user).to be_super_admin
    end
  end

  describe '#site_admin?' do
    it 'returns false when the user is not a site admin' do
      expect(user).not_to be_site_admin
    end

    it 'returns true when the user is in a site admin group' do
      user.ldap_group_string = 'sul:requests-site-admin'
      expect(user).to be_site_admin
    end
  end

  describe '#patron' do
    before do
      user.name = 'jstanford'
      user.email = 'jstanford@stanford.edu'
    end

    it 'returns placeholder patron for name email user' do
      expect(user.patron).to have_attributes(id: nil, display_name: 'jstanford', email: 'jstanford@stanford.edu', block_reasons: [])
    end
  end

  describe '#aeon' do
    context 'for non-SSO users' do
      it 'returns a NullUser (until we have a better idea how to authenticate them...)' do
        expect(user.aeon).to have_attributes(persisted?: false)
      end
    end

    context 'for SSO users' do
      before do
        user.sunetid = 'jstanford'
      end

      it 'returns a NullUser when there is no Aeon account' do
        allow(Aeon::User).to receive(:find_by).and_raise(AeonClient::NotFoundError)

        expect(user.aeon).to have_attributes(persisted?: false)
      end

      it 'returns an Aeon::User when there is an Aeon account' do
        allow(Aeon::User).to receive(:find_by).and_return(Aeon::User.new(username: 'jstanford', auth_type: 'Default'))

        expect(user.aeon).to have_attributes(username: 'jstanford', auth_type: 'Default', persisted?: true)
      end
    end
  end
end

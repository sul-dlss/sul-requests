...
RSpec.describe Illiad::Request do
  ...
  describe '.where' do
    context 'when successful, it retrieves transaction requests and ignores inactive requests' do
      ...
      it 'correctly returns request objects' do
        expect(described_class.where(user_id: patron_sunet_id).length).to be(2)
      end
    end
  end

  context 'when the request is ILLIAD hold/recall' do
    subject(:hold) { described_class.new(JSON.parse(hold_recall_result)) }

    let(:expiration_hold) { described_class.new(JSON.parse(expiration_result)) }

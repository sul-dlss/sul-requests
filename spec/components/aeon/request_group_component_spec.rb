...
RSpec.describe Aeon::RequestGroupComponent, type: :component do
  ...
  context 'with submitted ead requests' do
    ...
    before do
      allow(first_request).to receive_messages(cancelled?: false, completed?: false, saved_for_later?: false, submitted?: true)
      allow(second_request).to receive_messages(cancelled?: false, completed?: false, saved_for_later?: false, submitted?: true)
      render_inline(described_class.new(request_group:))
    end

    it 'renders information about each request' do
      expect(page).to have_css 'h2', text: 'Donald E. Knuth papers', count: 1
      expect(page).to have_css '.request-grid-id', text: 'The Art of Computer Programming', count: 2

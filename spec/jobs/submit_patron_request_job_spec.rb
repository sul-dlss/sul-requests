...
RSpec.describe SubmitPatronRequestJob do
  ...
  context 'when the item is a recall' do
    before do
      ...
      allow(folio_instance.items[0]).to receive_messages(
    ...
    context 'when the item is not in an easily recallable status' do
      before do
        allow(folio_instance.items[0]).to receive(:status).and_return(Folio::Item::STATUS_MISSING)
      end
    ...
    context 'when the item is in a library that we prefer to send to ILLiad' do
      before do
        allow(folio_instance.items[0]).to receive(:illiad_preferred?).and_return(true)
      end

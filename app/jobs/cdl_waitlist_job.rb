...
class CdlWaitlistJob < ApplicationJob
  ...
  def perform(circ_record_key, checkout_date:)
    ...
    if active_hold_records.any?
      ...
      check_for_symphony_errors(symphony_client.update_hold(active_hold_record.key, comment: comment))
    ...
    checkout = check_for_symphony_errors(
    ...
    check_for_symphony_errors(symphony_client.update_hold(next_up.key, comment: comment))

...
class AdminController < ApplicationController
  ...
  def generate_csv
    CSV.generate do |csv|
      csv << %w[year month type origin destination title requester]
  ...
  def csv_row(row)
    [row.created_at.year, row.created_at.month, row.type,
     row.folio_location&.name || row.origin_location_code,
     row.pickup_service_point&.name || row.service_point_code,
     row.item_title, row.patron_name || row.patron_email || row.patron_id]
  end

...
class PatronRequestsController < ApplicationController
  ...
  def new_params # rubocop:disable Metrics/AbcSize
    return patron_request_params if params[:patron_request].present?
  ...
  def transform_to_patron_request_item_params(patron_request_params) # rubocop:disable Metrics/MethodLength,Metrics/AbcSize,Metrics/CyclomaticComplexity,Metrics/PerceivedComplexity
    if patron_request_params[:ead_url].present?
      ...
      item_params = (patron_request_params[:item_ids] || []).compact_blank.map do |item_id|
        {
          item_id: item_id,
          ...
          activity_ids: patron_request_params[:activity_ids],
          **(patron_request_params[:aeon_item] || {})[item_id]&.except(:id)
        }
      end

...
module Folio
  module Format
    # DSL helpers for the FOLIO format rules. Copied from
    # searchworks_traject_indexer's lib/traject/macros/folio_format.rb
    # (plus literal / literal_multiple from macros/extras.rb).
    #
    # Each helper returns a lambda. Single-condition helpers take the shape
    #   ->(record, _context) { true/false }
    # Action helpers take the shape
    #   ->(record, accumulator, _context) { accumulator << 'value' }
    # `condition` and `all_conditions` glue them together.
    module Macros
      def literal(value)
      ...
      # Start of searchworks_traject_indexer folio_format.rb Traject::Macros::FolioFormat
      ...
      # End of searchworks_traject_indexer folio_format.rb Traject::Macros::FolioFormat

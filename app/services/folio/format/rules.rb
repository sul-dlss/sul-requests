...
module Folio
  module Format
    # Builds the array of format-rule lambdas that compute format_hsim.
    #
    # The body of #define_rules is copied verbatim from
    # searchworks_traject_indexer's lib/traject/config/folio_format_config.rb,
    # so the diff against that file should remain small and easy to resync when
    # upstream changes.
    #
    # `to_field 'format_hsim', <lambda>` is shimmed locally to push the lambda
    # onto @rules. Traject is not required at runtime.
    class Rules
      include Folio::Format::Macros

      def self.all
        @all ||= begin
          builder = new
          builder.define_rules
          builder.rules.freeze
        end
      end
      ...
      def to_field(_field, rule = nil, &block)
        @rules << (rule || block)
      end
